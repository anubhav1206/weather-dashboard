...
export const SearchBar = () => {
  ...
  return (
    <div className="flex justify-between items-center z-10 space-x-4">
      ...
      >
        <div className="fixed inset-0 bg-pink-300/20 backdrop-blur" />
        <Dialog.Panel className="relative shadow-lg bg-darkPurple overflow-y-auto ring-1 ring-cyan-700/10 rounded-xl px-4 py-3 w-[250px] xs:w-[380px] mx-auto">
          <Combobox value={selectedCity} by="key" onChange={setSelectedCity}>
            ...
            {searchResults.length > 0 && (
              <>
                ...
                >
                  {searchResults.map((res) => (
                    <Combobox.Option
                      key={res.key}
                      value={res}
                      className="cursor-pointer rounded-md w-full py-3 px-2 text-darkGrey font-satoshi text-sm sm:text-base ui-active:bg-[#c77e6b] hover:border-2 overflow-hidden flex justify-between items-center"
                    >
                      {({ selected }) => (
                        <>
                ...
                <motion.button
                    className="px-3 py-2 ml-32 rounded-md font-satoshi text-base  sm:text-lg bg-[#c77e6b] text-darkGrey font-medium relative overflow-hidden flex items-end"
                    onClick={() => setSubmitted((prev) => !prev)}
                    whileTap={{ scale: 0.9, backgroundColor: "#a9534e" }}
                  >
                    <motion.div
                      className="absolute inset-0 border-2 border-beige rounded-md flex animation-safelist"
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      exit={{ scale: 0 }}
